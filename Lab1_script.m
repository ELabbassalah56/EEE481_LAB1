...
rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_1');  % Decode the received bits

% Compute the BER
BER_case_1 = ComputeBER(bit_seq,rec_bit_seq); % Calculate the bit error rate
...
title('The Ber Vs probability of bit flipping Part1')
ylabel('berVect')
xlabel('pVect')
%%%
...
title('The Ber Vs probability of bit flipping Part2')
ylabel('berVect')
xlabel('pVect')
%%%
...
title('The Ber Vs probability of bit flipping Part3')
ylabel('berVect')
xlabel('pVect')
% Plotting results
...
fs_vect          = 1:6:50;              % Use this vector to extract different values of fs in your code  
BER_case_4_vec  = zeros(size(fs_vect));                % Use this vector to store the resultant BER
% Generate a bit sequence
bit_seq = GenerateBits(N_bits); % Generate a sequence of bits equal to the total number of bits

for i_count=1 : length(fs_vect)
% Generate samples from bits
sample_seq = GenerateSamples(bit_seq,fs_vect(i_count)); % Generate a sequence of samples for each bit

% Pass the sample sequence through the channel
rec_sample_seq  = BSC(sample_seq,fs_vect(i_count),p); % Generate the received samples after passing through the bit flipping channel

% Decode bits from received bit sequence
rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_2',fs_vect(i_count));    % IMPLEMENT THIS: Decode the received bits

% Compute the BER
BER_case_4(i_count) = ComputeBER(bit_seq,rec_bit_seq);  % Calculate the bit error rate
end
figure
plot(fs_vect,BER_case_4_vec);
title('The Ber Vs repeated Sample Part4')
ylabel('berVect')
xlabel('fsVect')
